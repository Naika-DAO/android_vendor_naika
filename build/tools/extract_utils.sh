...
function setup_vendor() {
    ...
    export NAIKA_ROOT="$3"
    if [ ! -d "$NAIKA_ROOT" ]; then
        echo "\$NAIKA_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$NAIKA_ROOT/$OUTDIR" ]; then
        mkdir -p "$NAIKA_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$NAIKA_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$NAIKA_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$NAIKA_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$NAIKA_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$NAIKA_ROOT"/prebuilts/tools-dot/common/smali/baksmali.jar
        export SMALIJAR="$NAIKA_ROOT"/prebuilts/tools-dot/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$NAIKA_ROOT"/prebuilts/tools-dot/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$NAIKA_ROOT"/prebuilts/tools-dot/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$NAIKA_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$NAIKA_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$NAIKA_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$NAIKA_ROOT"/vendor/naika/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$NAIKA_ROOT"/"$OUTDIR"/radio
